...
if (cluster.isMaster) {
  ...
  redisClient.hmset(defaultSession + "-session", { 'name': defaultSession, 'corpus': defaultCorpus }, (err, reply) => {
    // console.log(data); //TODO: Figure out why this throws an error?
  });
  ...
  io.sockets.on('connection', function(socket) {
    ...
    socket.on('addme', function(data) {
      ...
      if (username == "theater") {
        let theaterID = socket.id;
        redisClient.set("theaterID-" + socket.sessionName, theaterID);
        console.log("Hello Theater: " + theaterID);
      }

      if (username == "installation") {
        let installationID = socket.id;
        redisClient.set("installationID-" + socket.sessionName, installationID);
        console.log("Hello Installation: " + installationID);
      }

      if (username == "controller") {
        let controllerID = socket.id;
        redisClient.set("controllerID-" + socket.sessionName, controllerID);
        console.log("Hello Controller: " + controllerID);
      }

      if (username == "audio_controller") {
        let audioControllerID = socket.id;
        redisClient.set("audioControllerID-" + socket.sessionName, audioControllerID);
    ...
    socket.on('joinSession', function(data) {
      ...
      redisClient.hget(sessionName + "-session", 'corpus', (err, reply) => {
        ...
        io.to(socket.id).emit('registerComplete', {
          'greeting': `SERVER: You have connected. Hello: ${socket.username} ID: ${socket.id} Color: ${socket.userColor}`,
